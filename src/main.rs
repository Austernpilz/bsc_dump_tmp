...
mod random;
use crate::random::create_random_fasta;
...
fn main() {
    ...
    match args.command {
        ...
        Commands::Random (mut rand_args) => {
            println!("Random Generating");
            if rand_args.lens.len() == 0 {
                rand_args.lens.push(1000);
            }
            create_random_fasta(&rand_args);
        }
    }
}
